...
func TestNormalize_DryRun_Runner결과를출력한다(t *testing.T) {
	...
	command := newNormalizeCommand(func() config.Config { return config.Config{} }, func(_ context.Context, _ config.Config, received normalization.Command) (normalization.Summary, error) {
		...
		}, RemainingPending: 3, RemainingStale: 2, IdentityFilled: 4, Inherited: 5, InheritanceReleased: 6, InheritanceConflicts: 7, AlcoholNamesApplied: 8}, nil
	}, output)
	...
	want := "normalize 결과: normalized=0 partial=1 review_required=0 unparsed=0 system_failures=0 remaining_pending=3 remaining_stale=2 identity_filled=4 inherited=5 inheritance_released=6 inheritance_conflicts=7 alcohol_names_applied=8 dry_run=true\n"
