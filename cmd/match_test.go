...
func TestMatchCommand_AllDryRun_옵션과요약을전달한다(t *testing.T) {
	...
	command := newMatchCommand(func() config.Config { return config.Config{} }, func(
		...
		received = value
		return usecase.Summary{Processed: 12, AlcoholMatched: 9, DistilleryMatched: 8, RegionMatched: 7, NoMatch: 3, Remaining: 0, MatchingVersion: "v1:hash"}, nil
	}, out)
	...
	if !strings.Contains(out.String(), "processed=12") || !strings.Contains(out.String(), "alcohol_matched=9") || !strings.Contains(out.String(), "version=v1:hash") {
