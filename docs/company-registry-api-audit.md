...
| I0470 | 성공 | 성공 | pagination 중 전체 건수 변동 확인 |
...
정상 응답은 `{SERVICE_ID:{total_count:"string",row:[],RESULT:{MSG,CODE}}}` 구조다. C001 81,805행과 I0250 59행은 안정된 `total_count` 기준으로 끝까지 조회했다. I0470은 5,444행을 조회하는 동안 페이지별 `total_count`가 5,444와 5,452로 바뀌어 일관된 전체 snapshot을 확정하지 못했다. I2821은 100페이지·100,000행까지 모든 페이지가 1,000행이어서 전체 건수를 확정하지 못했다. I2821 `total_count`는 요청 범위가 1, 2, 5일 때 각각 1, 2, 5를 반환하므로 종료 조건으로 사용하지 않는다.
