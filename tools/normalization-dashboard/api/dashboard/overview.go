...
const coverageSQL = `SELECT field_name, populated, total FROM (SELECT 'normalized_name' AS field_name, SUM(COALESCE(sku_display_name_ko, sku_display_name_en, '') <> '') AS populated, COUNT(*) AS total FROM mfds_declaration_details UNION ALL SELECT 'volume', SUM(volume_ml IS NOT NULL), COUNT(*) FROM mfds_declaration_details UNION ALL SELECT 'abv', SUM(abv_percent IS NOT NULL), COUNT(*) FROM mfds_declaration_details UNION ALL SELECT 'ingredient_percent', SUM(declaration_v3.ingredient_percent_raw IS NOT NULL), COUNT(*) FROM ` + declarationV3BaseSourceSQL + ` UNION ALL SELECT 'variant_marker', SUM(declaration_v3.variant_marker_raw IS NOT NULL), COUNT(*) FROM ` + declarationV3BaseSourceSQL + ` UNION ALL SELECT 'age', SUM(age_years IS NOT NULL), COUNT(*) FROM mfds_declaration_details UNION ALL SELECT 'vintage', SUM(vintage_year IS NOT NULL), COUNT(*) FROM mfds_declaration_details UNION ALL SELECT 'importer', SUM(COALESCE(importer_base_name, '') <> ''), COUNT(*) FROM mfds_declaration_details) coverage ORDER BY field_name`
